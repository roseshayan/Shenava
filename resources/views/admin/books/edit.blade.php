...
@section('content')
    ...
    <form action="{{ route('books.update', $book->id) }}" method="POST" class="bg-white p-6 rounded shadow-md">
            ...
            <select name="author_id" class="border px-3 py-2 w-full rounded" required>
                @foreach($authors as $author)
                    <option value="{{ $author->id }}"
                            @if($book->author_id==$author->id) selected @endif>{{ $author->name }}</option>
                @endforeach
            </select>
        </div>
            ...
            <select name="category_id" class="border px-3 py-2 w-full rounded">
                @foreach($categories as $category)
                    <option value="{{ $category->id }}"
                            @if($book->category_id==$category->id) selected @endif>{{ $category->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="description" class="border px-3 py-2 w-full rounded"
        ...
        <button type="submit" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded">ذخیره تغییرات
        </button>
    </form>
@endsection
